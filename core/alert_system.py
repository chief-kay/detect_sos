...
class AlertSystem:
    ...
    def _validate_config(self):
        ...
        if not self.api_key:
            self.logger.error(
                "Arkesel API key not provided. Set ARKESEL_API_KEY environment variable.")
            raise ValueError("Missing Arkesel API key")

        if not self.sender_id:
            self.logger.warning(
                "No sender ID provided, using default 'UAVDRONE'")
    ...
    def send_sms(self, phone_numbers: List[str], message: str, priority: str = 'medium') -> Dict:
        ...
        try:
            ...
            if response_data['status'] == 'success':
                self.logger.info(
                    f"SMS sent successfully to {len(phone_numbers)} recipients")
                return {
                    'success': True,
    ...
    def get_contacts_for_event(self, event_type: str, priority: str = 'medium') -> List[str]:
        ...
        contact_groups = self.priority_contacts.get(
            priority, ['control_center'])
        ...
        with self.rate_limit_lock:
            ...
            if alert_key in self.last_alert_time:
                time_since_last = current_time - \
                    self.last_alert_time[alert_key]
                if time_since_last < self.min_alert_interval:
                    self.logger.info(
                        f"Rate limiting: {alert_key} alert suppressed")
                    return False
    ...
    def send_test_alert(self, test_numbers: List[str] | None = None) -> Dict:
        ...
        test_contacts = test_numbers or [
            '233592876626']  # Add your test number
    ...
    def remove_emergency_contact(self, group: str, phone_number: str):
        ...
        if group in self.emergency_contacts and phone_number in self.emergency_contacts[group]:
            ...
            self.logger.info(
                f"Removed contact {phone_number} from group {group}")

    def get_stats(self) -> Dict:
        """Get alert system statistics"""
        successful_alerts = len(
            [a for a in self.alert_history if a['success']])
